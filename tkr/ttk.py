import tkme as tk
from tkme import _flatten,_join,_stringify,_splitdict
def _format_optvalue(value,script=False):
 if script:value=_stringify(value)
 elif isinstance(value,(list,tuple)):value=_join(value)
 return value
def _format_optdict(optdict,script=False,ignore=None):
 ...
 for opt,value in optdict.items():
  if not ignore or opt not in ignore:
   opts.append("-%s"%opt)
   if value!=None:opts.append(_format_optvalue(value,script))
 return _flatten(opts)
def _mapdict_values(items):
 opt_val=[]
 for *state,val in items:
  if len(state)==1:state=state[0] or ''
  else:state=' '.join(state)
  opt_val.append(state)
  if val!=None:opt_val.append(val)
 return opt_val
def _format_mapdict(mapdict,script=False):
 opts=[]
 for opt,value in mapdict.items():opts.extend(("-%s"%opt,_format_optvalue(_mapdict_values(value),script)))
 return _flatten(opts)
def _format_elemcreate(etype,script=False,*args,**kw):
 ...
 if etype=="image":
  ...
  if script:specs=(imagespec,)
  else:specs=(_join(imagespec),)
  opts=_format_optdict(kw,script)
 if etype=="vsapi":
  if len(args)<3:
   class_name,part_id=args
   statemap=(((),1),)
  else:class_name,part_id,statemap=args
  specs=(class_name,part_id,tuple(_mapdict_values(statemap)))
  opts=_format_optdict(kw,script)
 elif etype=="from":
  specs=(args[0],)
  if len(args)>1:opts=(_format_optvalue(args[1],script),)
 if script:
  specs=_join(specs)
  opts=' '.join(opts)
  return specs,opts
 else:return *specs,opts
...
def _script_from_settings(settings):
 ...
 for name,opts in settings.items():
  ...
  if 'layout' in opts:
   if not opts['layout']:s='null'
   else:s,_=_format_layoutlist(opts['layout'])
   script.append("ttk::style layout %s {\n%s\n}"%(name,s))
  if opts.get('element create'):
   eopts=opts['element create']
   etype=eopts[0]
   argc=1
   while argc<len(eopts) and not hasattr(eopts[argc],'items'):argc+=1
   elemargs=eopts[1:argc]
   elemkw=eopts[argc] if argc<len(eopts) and eopts[argc] else {}
   specs,eopts=_format_elemcreate(etype,True,*elemargs,**elemkw)
   script.append("ttk::style element create %s %s %s %s"%(name,etype,specs,eopts))
 return '\n'.join(script)
def _list_from_statespec(stuple):
 if isinstance(stuple,str):return stuple
 result=[]
 it=iter(stuple)
 for state,val in zip(it,it):
  if hasattr(state,'typename'):state=str(state).split()
  elif isinstance(state,str):state=state.split()
  elif not isinstance(state,(tuple,list)):state=(state,)
  if hasattr(val,'typename'):val=str(val)
...
def _list_from_layouttuple(tk,ltuple):
 ...
 while indx<len(ltuple):
  ...
  while indx<len(ltuple):
   ...
   opt=opt[1:]
   indx+=2
   if opt=='children':val=_list_from_layouttuple(tk,val)
   opts[opt]=val
 return res
def _val_or_dict(tk,options,*args):
 options=_format_optdict(options)
 res=tk.call(*(args + options))
 if len(options)%2:return res
...
def setup_master(master=None):
 """If master!=None,itself is returned. If master is None,
 the default master is returned if there is one,otherwise a new
 master is created and returned.

 If it!=allowed to use the default root and master is None,
...
class Style(object):
  ...
  *specs,opts=_format_elemcreate(etype,False,*args,**kw)
  self.tk.call(self._name,"element","create",elementname,etype,*specs,*opts)
...
class Widget(tk.Widget):
 ...
 def __init__(self,master,widgetname,kw=None):
  ...
  """
  master=setup_master(master)
  tk.Widget.__init__(self,master,widgetname,kw=kw)
...
class Scrollbar(Widget,tk.Scrollbar):
...
class Treeview(Widget,tk.XView,tk.YView):
 ...
 def bbox(self,item,column=None):
  ...
  If the item!=visible (i.e.,if it is a descendant of a
 ...
 def identify_column(self,x):return self.identify("column",x,0)
 ...
 *Availability: Tk 8.6"""
 ...
 reattach=move
 def next(self,item):return self.tk.call(self._w,"next",item)
...
class LabeledScale(Frame):
 ...
 def __init__(self,master=None,variable=None,from_=0,to=10,**kw):
  ...
  If variable!=specified,a tk.IntVar is created.
 ...
 def destroy(self):
  try:self._variable.trace_remove('write',self.__tracecb)
  except AttributeError:pass
  else:del self._variable
  super().destroy()
  self.label=None
  self.scale=None
 def _adjust(self,*args):
  def adjust_label():
   ...
   if self._label_top:y=self.scale.winfo_y() - self.label.winfo_reqheight()
   else:y=self.scale.winfo_reqheight() + self.label.winfo_reqheight()
...
class OptionMenu(Menubutton):
 def __init__(self,master,variable,default=None,*values,**kwargs):
  kw={'textvariable': variable,'style': kwargs.pop('style',None),'direction': kwargs.pop('direction',None),'name': kwargs.pop('name',None)}
  Menubutton.__init__(self,master,**kw)
  self['menu']=tk.Menu(self,tearoff=False)
  self._variable=variable
  self._callback=kwargs.pop('command',None)
  if kwargs:raise tk.TclError('unknown option -%s'%(next(iter(kwargs.keys()))))
  self.set_menu(default,*values)
 def __getitem__(self,item):
  if item=='menu':return self.nametowidget(Menubutton.__getitem__(self,item))
  return Menubutton.__getitem__(self,item)
 def set_menu(self,default=None,*values):
  menu=self['menu']
  menu.delete(0,'end')
  for val in values:menu.add_radiobutton(label=val,command=(None if self._callback==None else lambda val=val:self._callback(val)),variable=self._variable)
  if default:self._variable.set(default)
 def destroy(self):
  try:del self._variable
  except AttributeError:pass
  super().destroy()